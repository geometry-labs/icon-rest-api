...
def test_get_block_by_height(prep_fixtures, client: TestClient) -> None:
    ...
    r = client.get(f"{settings.ICON_REST_API_PREFIX}/blocks/height/{latest_height}")
    response = r.json()
    assert r.status_code == 200
...
def test_get_block_by_height_error(prep_fixtures, client: TestClient) -> None:
    r = client.get(f"{settings.ICON_REST_API_PREFIX}/blocks/height/1000000000000000")
    response = r.json()
...
def test_get_block_by_hash(prep_fixtures, client: TestClient) -> None:
    ...
    r = client.get(
        f"{settings.ICON_REST_API_PREFIX}/blocks/hash/{latest_hash}"
    )
    response = r.json()
    assert r.status_code == 200
...
def test_get_block_by_hash_error(prep_fixtures, client: TestClient) -> None:
    r = client.get(
        f"{settings.ICON_REST_API_PREFIX}/blocks/hash/1111111111111111111111fbcc7d17f5983570e8ddff6aff594d4dd44059"
    )
    response = r.json()
    assert r.status_code == 404
    assert response == {"detail": "Block not found"}
